...
export function calculateTargetReturn({
  ...
  const requiredAnnualReturn =
    Math.pow(targetAsset / currentAsset, 1 / years) - 1;
...
export function calculateDCA({
  ...
  const scenarios = {
    pessimistic: calculateDCAScenario(
      monthlyInvestment,
      years,
      pessimisticReturn
    ),
    realistic: estimatedValue,
    optimistic: calculateDCAScenario(
      monthlyInvestment,
      years,
      optimisticReturn
    ),
...
export function calculateRetirement({
  ...
  for (let year = 0; year < retirementYears; year++) {
    const yearlyExpense =
      futureMonthlyExpense * 12 * Math.pow(1 + inflationRate / 100, year);
  ...
  const futureCurrentAsset =
    currentAsset * Math.pow(1 + expectedReturn / 100, yearsToRetirement);
  ...
  if (currentShortfall > 0) {
    if (monthlyReturn > 0) {
      monthlyContributionNeeded =
        (currentShortfall * monthlyReturn) /
