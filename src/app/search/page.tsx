...
export default function SearchPage() {
  ...
  >('all');
  const [filingsFilter, setFilingsFilter] = useState<
    'all' | '10-K' | '10-Q' | '8-K'
  >('all');
  ...
  const fetchStockData = async (symbol: string) => {
    ...
    try {
      const response = await fetch(
        `/api/stock-data?symbols=${symbol}&history=true&days=30`
      );
  ...
  const loadSummary = useCallback(
    async (symbol: string) =>
      withSectionLoader('summary', async () => {
        const r = await fetch(
          `/api/summary?symbols=${symbol}&modules=price,summaryDetail,financialData,assetProfile`
        );
        const j = await r.json();
  ...
  const loadFinancials = useCallback(
    async (symbol: string, period: 'annual' | 'quarterly') =>
      withSectionLoader('financials', async () => {
        const r = await fetch(
          `/api/financials?symbols=${symbol}&period=${period}`
        );
        const j = await r.json();
  ...
  }, [financialsPeriod, selectedStock?.symbol, loadFinancials]);
  ...
  const sortedNews = useMemo(() => {
    ...
    if (newsSort === 'latest') {
      items.sort(
        (a: any, b: any) =>
          new Date(b.providerPublishTime || b.publishedAt || 0).getTime() -
          new Date(a.providerPublishTime || a.publishedAt || 0).getTime()
      );
    } else {
  ...
  const filteredRatings = useMemo(() => {
    ...
    if (ratingsFilter === 'upgrade')
      return items.filter((r: any) =>
        String(r.action || '')
          .toLowerCase()
          .includes('up')
      );
    if (ratingsFilter === 'downgrade')
      return items.filter((r: any) =>
        String(r.action || '')
          .toLowerCase()
          .includes('down')
  ...
  const filteredFilings = useMemo(() => {
    const items: any[] =
      (filingsData?.filings ?? filingsData?.filingsRecent ?? []) || [];
    if (filingsFilter === 'all') return items;
    return items.filter((f: any) =>
      (f?.type || f?.form || '').toUpperCase().includes(filingsFilter)
    );
            ...
            <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex justify_between items-center">
                      ...
                      <span
                        className={`text-lg font-medium ${getChangeColor(selectedStock.change)}`}
                      >
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {selectedStock.peRatio
                            ? formatNumber(selectedStock.peRatio, 2)
                            : 'N/A'}
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {selectedStock.dividendYield
                            ? `${formatNumber(selectedStock.dividendYield, 2)}%`
                            : 'N/A'}
                        </span>
                      </div>
                      <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {errorMap.earnings && (
                      <div className="text-sm text-red-500 flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            selectedStock && loadEarnings(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                      ...
                      earningsData && (
                        <div className="space-y-2 text-sm">
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {earningsData?.earningsHistory?.history?.[0]
                                ?.epsActual ?? 'N/A'}
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {earningsData?.earningsTrend?.trend?.[0]?.epsTrend
                                ?.current ?? 'N/A'}
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {earningsData?.calendarEvents?.earnings
                                ?.earningsDate?.[0]
                                ? new Date(
                                    earningsData.calendarEvents.earnings.earningsDate[0]
                    ...
                    {errorMap.analyst && (
                      <div className="text-sm text-red-500 flex items_center gap-2">
                        ...
                        <button
                          className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                          onClick={() =>
                            selectedStock && loadAnalyst(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                      ...
                      analystData && (
                        <div className="space-y-2 text-sm">
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {analystData?.recommendationTrend?.trend?.[0]
                                ?.strongBuy ?? 0}{' '}
                              Strong Buy
                            </span>
                          </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    {loadingMap.ownership && (
                      <p className="text_sm text-gray-500">불러오는 중...</p>
                    )}
                    {errorMap.ownership && (
                      <div className="text-sm text-red-500 flex items-center gap-2">
                        ...
                        <button
                          className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                          onClick={() =>
                            selectedStock && loadOwnership(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                      ...
                      ownershipData && (
                        <div className="space-y-2 text-sm">
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {ownershipData?.majorHoldersBreakdown
                                ?.institutionsPercentHeld
                                ? `${formatNumber(ownershipData.majorHoldersBreakdown.institutionsPercentHeld * 100, 2)}%`
                                : 'N/A'}
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {ownershipData?.majorHoldersBreakdown
                                ?.insidersPercentHeld
                                ? `${formatNumber(ownershipData.majorHoldersBreakdown.insidersPercentHeld * 100, 2)}%`
                                : 'N/A'}
                            </span>
                          </div>
                        </div>
                    ...
                    {errorMap.summary && (
                      <div className="text-sm text-red-500 flex items-center gap-2">
                        ...
                        <button
                          className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                          onClick={() =>
                            selectedStock && loadSummary(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                      ...
                      summaryData && (
                        <div className="space-y-2 text-sm">
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {summaryData?.assetProfile?.sector ?? 'N/A'}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-600 dark:text-gray-400">
                            ...
                            <span className="ml-2 font-medium">
                              {summaryData?.assetProfile?.industry ?? 'N/A'}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-600 dark:text-gray-400">
                            ...
                            <span className="ml-2 font-medium">
                              {summaryData?.assetProfile?.fullTimeEmployees ??
                                'N/A'}
                            </span>
                          </div>
                        </div>
                    ...
                    {errorMap.insights && (
                      <div className="text-sm text-red-500 flex items-center gap-2">
                        ...
                        <button
                          className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                          onClick={() =>
                            selectedStock && loadInsights(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                    ...
                    {!loadingMap.insights && !errorMap.insights && (
                      <>
                        <ul className="list-disc list-inside text-sm space-y-1">
                          ...
                          {sortedNews.length === 0 && (
                            <li className="text-gray-500">뉴스 없음</li>
                          )}
                        </ul>
                        {sortedNews.length > newsLimit && (
                          <button
                            className="mt-3 text-sm px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded"
                            onClick={() => setNewsLimit(v => v + 5)}
                          >
                            더 보기
                          </button>
                        )}
                  ...
                  <CardContent>
                    ...
                    <div className="mb-3 flex items-center gap-2 text-sm">
                      ...
                      <select
                        className="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1"
                        value={ratingsFilter}
                        onChange={e => setRatingsFilter(e.target.value as any)}
                      >
                    ...
                    {errorMap.ratings && (
                      <div className="text-sm text-red-500 flex items-center gap-2">
                        ...
                        <button
                          className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                          onClick={() =>
                            selectedStock && loadRatings(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                    ...
                    {!loadingMap.ratings && !errorMap.ratings && (
                      <>
                        <ul className="list-disc list-inside text-sm space-y-1">
                          {filteredRatings
                            .slice(0, ratingsLimit)
                            .map((r: any, idx: number) => (
                              <li key={idx}>
                                {r.firm}: {r.fromGrade} → {r.toGrade} (
                                {r.action})
                              </li>
                            ))}
                  ...
                  <CardContent>
                    ...
                    <div className="mb-3 flex items-center gap-2 text-sm">
                      ...
                      <select
                        className="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1"
                        value={filingsFilter}
                        onChange={e => setFilingsFilter(e.target.value as any)}
                      >
                        <option value="all">전체</option>
                    ...
                    {errorMap.filings && (
                      <div className="text-sm text-red-500 flex items-center gap-2">
                        ...
                        <button
                          className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                          onClick={() =>
                            selectedStock && loadFilings(selectedStock.symbol)
                          }
                        >
                          재시도
                        </button>
                    ...
                    {!loadingMap.filings && !errorMap.filings && (
                      <>
                        <ul className="list-disc list-inside text-sm space-y-1">
                          {filteredFilings
                            .slice(0, filingsLimit)
                            .map((f: any, idx: number) => (
                              <li key={idx}>
                                {f?.type || f?.form} •{' '}
                                {f?.date || f?.fillingDate}
                                {f?.edgarUrl && (
                                  <a
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <select
                        ...
                        onChange={e =>
                          setFinancialsPeriod(
                            e.target.value as 'annual' | 'quarterly'
                          )
                        }
                      >
                        ...
                        onClick={() => {
                          const isRows = (
                            financialsPeriod === 'annual'
                              ? financialsData?.incomeStatementHistory
                                  ?.incomeStatementHistory || []
                              : financialsData?.incomeStatementHistoryQuarterly
                                  ?.incomeStatementHistory || []
                          ).slice(0, 12);
                          ...
                          downloadCsv(
                            `${selectedStock?.symbol || 'symbol'}_${financialsPeriod}_income.csv`,
                            rows
                          );
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={() => {
                          const bsRows = (
                            financialsPeriod === 'annual'
                              ? financialsData?.balanceSheetHistory
                                  ?.balanceSheetStatements || []
                              : financialsData?.balanceSheetHistoryQuarterly
                                  ?.balanceSheetStatements || []
                          ).slice(0, 12);
                          const rows: Array<string[]> = [
                            [
                          ...
                          downloadCsv(
                            `${selectedStock?.symbol || 'symbol'}_${financialsPeriod}_balance.csv`,
                            rows
                          );
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={() => {
                          const cfRows = (
                            financialsPeriod === 'annual'
                              ? financialsData?.cashflowStatementHistory
                                  ?.cashflowStatements || []
                              : financialsData
                          ...
                          cfRows.forEach((row: any) => {
                            const cfo =
                              row?.totalCashFromOperatingActivities ?? '';
                          ...
                          downloadCsv(
                            `${selectedStock?.symbol || 'symbol'}_${financialsPeriod}_cashflow.csv`,
                            rows
                          );
                        }}
                      >
                              ...
                              .map((row: any, idx: number) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="py-2 pr-4">
                                      {icr != null ? icr.toFixed(2) : 'N/A'}
                                    </td>
                                    <td className="py-2 pr-4">
                                      {net != null ? formatVolume(net) : 'N/A'}
                                    </td>
                                  </tr>
                                );
                              })}
                        ...
                        <table className="min-w-full text-sm">
                          ...
                          <tbody>
                            {(financialsPeriod === 'annual'
                              ...
                              .map((row: any, idx: number) => {
                                const cfo =
                                  typeof row?.totalCashFromOperatingActivities ===
                                  'number'
                                    ? row.totalCashFromOperatingActivities
                                    : null;
                                const capex =
                                  typeof row?.capitalExpenditures === 'number'
                  ...
                  {errorMap.keyStats && (
                    <div className="text-sm text-red-500 flex items-center gap-2">
                      ...
                      <button
                        className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                        onClick={() =>
                          selectedStock && loadKeyStats(selectedStock.symbol)
                        }
                      >
                        재시도
                      </button>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        <div>
                          ...
                          <span className="font-medium">
                            {formatVolume(
                              keyStatsData?.price?.marketCap ||
                                keyStatsData?.financialData?.marketCap
                            )}
                          </span>
                        </div>
                        <div>
                          베타:{' '}
                          <span className="font-medium">
                            {keyStatsData?.defaultKeyStatistics?.beta ?? 'N/A'}
                          </span>
                        </div>
                        <div>
                          PSR(TTM):{' '}
                          <span className="font-medium">
                            {keyStatsData?.defaultKeyStatistics
                        ...
                        <div>
                          PER(TTM):{' '}
                          <span className="font-medium">
                            {keyStatsData?.defaultKeyStatistics?.trailingPE ??
                              keyStatsData?.financialData?.trailingPE ??
                              'N/A'}
                          </span>
                        </div>
                        <div>
                        ...
                        <div>
                          배당수익률:{' '}
                          <span className="font-medium">
                            {keyStatsData?.summaryDetail?.dividendYield
                              ? `${(keyStatsData.summaryDetail.dividendYield * 100).toFixed(2)}%`
                              : 'N/A'}
                          </span>
                        </div>
                      </div>
                  ...
                  {errorMap.options && (
                    <div className="text-sm text-red-500 flex items-center gap-2">
                      ...
                      <button
                        className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                        onClick={() =>
                          selectedStock && loadOptions(selectedStock.symbol)
                        }
                      >
                        재시도
                      </button>
                    ...
                    optionsData.nearest && (
                      <div className="overflow-x-auto text-sm">
                        <div className="mb-2 text-gray-600 dark:text-gray-400">
                          만기:{' '}
                          {optionsData.nearest.expirationDate
                            ? new Date(optionsData.nearest.expirationDate)
                                .toISOString()
                                .split('T')[0]
                            : 'N/A'}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <table className="min-w-full">
                              <thead>
                                <tr className="text-left">
                                  <th className="py-1 pr-3">행사가</th>
                                  <th className="py-1 pr-3">가격</th>
                                  <th className="py-1 pr-3">OI</th>
                                </tr>
                              </thead>
                              <tbody>
                                {(optionsData.nearest.calls || []).map(
                                  (c: any, i: number) => (
                                    <tr
                                      key={`c-${i}`}
                                      className="border-t border-gray-200 dark:border-gray-700"
                          ...
                          <div>
                            ...
                            <table className="min-w-full">
                              ...
                              <tbody>
                                {(optionsData.nearest.puts || []).map(
                                  (p: any, i: number) => (
                                    <tr
                                      key={`p-${i}`}
                                      className="border-t border-gray-200 dark:border-gray-700"
                                    >
                                      <td className="py-1 pr-3">{p.strike}</td>
                                      <td className="py-1 pr-3">
                                        {p.lastPrice ?? '-'}
                                      </td>
                                      <td className="py-1 pr-3">
                  ...
                  {errorMap.fundFacts && (
                    <div className="text-sm text-red-500 flex items-center gap-2">
                      ...
                      <button
                        className="px-2 py-1 bg-red-100 dark:bg-red-900/30 rounded"
                        onClick={() =>
                          selectedStock && loadFundFacts(selectedStock.symbol)
                        }
                      >
                        재시도
                      </button>
                      ...
                      <div className="text-sm space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div>
                            펀드 패밀리:{' '}
                            <span className="font-medium">
                              {fundFactsData?.fundProfile?.family ?? 'N/A'}
                            </span>
                          </div>
                          <div>
                            총보수(Expense Ratio):{' '}
                            <span className="font-medium">
                              {fundFactsData?.fundProfile
                                ?.feesExpensesInvestment
                                ?.annualReportExpenseRatio ?? 'N/A'}
                            </span>
                          </div>
                          <div>
                            운용온기:{' '}
                          ...
                          <div className="mb-2">
                            <button
                              ...
                              onClick={() => {
                                ...
                                (fundFactsData?.topHoldings?.holdings || [])
                                  .slice(0, 50)
                                  .forEach((h: any) =>
                                    rows.push([
                                      String(h.holdingName ?? ''),
                                      String(h.symbol ?? ''),
                                      h.holdingPercent != null
                                        ? String(h.holdingPercent)
                                        : '',
                                    ])
                                  );
                                downloadCsv(
                                  `${selectedStock?.symbol || 'fund'}_holdings.csv`,
                                  rows
                                );
                              }}
                            >
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            <div>
                              YTD:{' '}
                              <span className="font-medium">
                                {fundFactsData?.fundPerformance?.trailingReturns
                                  ?.returns?.ytd ?? 'N/A'}
                              </span>
                            </div>
                            <div>
