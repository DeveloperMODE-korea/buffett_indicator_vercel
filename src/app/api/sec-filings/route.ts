...
let cikMapCache: { map: Record<string, string>; expiresAt: number } | null =
  null;
...
async function loadCikMap(): Promise<Record<string, string>> {
  ...
  Object.values(json as any).forEach((row: any) => {
    if (row?.ticker && row?.cik_str) {
      map[String(row.ticker).toUpperCase()] = String(row.cik_str).padStart(
        10,
        '0'
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const results = await Promise.all(
      symbols.map(async (symbol: string) => {
        ...
        try {
          ...
          if (subs && subs.length) {
            return {
              success: true,
              symbol,
              data: { filings: subs, filingsRecent: [] },
            };
          }
        } catch (e) {
          // 무시하고 다음 폴백
        ...
        try {
          ...
          const yf = (res as any)?.secFilings || {
            filings: [],
            filingsRecent: [],
          };
        ...
        try {
          ...
          if (atom.length)
            return {
              success: true,
              symbol,
              data: { filings: atom, filingsRecent: [] },
            };
        } catch (e) {
          // 무시
        }
        ...
        return {
          ...
          data: {
            filings: [],
            filingsRecent: [],
            reason: 'no_data_from_all_sources',
          },
        };
      })
    );
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'SEC/공시 데이터를 가져오는 중 오류가 발생했습니다.',
      },
      { status: 500 }
    );
  }
