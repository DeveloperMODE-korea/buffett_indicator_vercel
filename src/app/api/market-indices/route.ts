...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ indices 타입 고정
    const indices: string[] = searchParams.get('indices')?.split(',') || [
      '^GSPC', // S&P 500
      '^DJI', // Dow Jones
      '^IXIC', // NASDAQ
      '^RUT', // Russell 2000
        ...
        try {
          ...
          if (includeHistory) {
            try {
              ...
              console.error(
                `[Market Indices API] ${symbol} 히스토리 데이터 오류:`,
                historyError
              );
              indexInfo.history = [];
            }
    ...
    console.log(
      `[Market Indices API] 성공: ${successfulData.length}개, 실패: ${failedData.length}개`
    );
