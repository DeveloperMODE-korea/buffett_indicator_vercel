  ...
  try {
    ...
    const processedResults =
      searchResults.quotes
        ?.filter(
          quote => quote && typeof quote === 'object' && 'symbol' in quote
        )
        .map(quote => ({
    ...
    const exactMatches = processedResults.filter(
      result => result.symbol.toUpperCase() === query.toUpperCase()
    );
    const otherMatches = processedResults.filter(
      result => result.symbol.toUpperCase() !== query.toUpperCase()
    );
