...
async function calculateMarketGDPRatio(): Promise<
...
> {
  try {
    ...
    for (const sp500Point of sp500Data) {
      const matchingGdp = gdpData.find(
        gdpPoint => gdpPoint.date === sp500Point.date
      );
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      'FRED API KEY (first 10 chars):',
      FRED_API_KEY?.substring(0, 10)
    );
