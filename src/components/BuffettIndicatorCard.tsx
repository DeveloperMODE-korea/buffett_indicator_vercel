...
export default function BuffettIndicatorCard({ data, loading, error }: Props) {
    ...
    return (
      <div className="card p-8 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 transition-colors">
        <div className="text-center">
          ...
          <div className="text-red-500 dark:text-red-300 transition-colors">
            {error}
          </div>
        </div>
      </div>
  ...
  return (
    <div
      className={`card p-8 ${statusInfo.bgColor} ${statusInfo.borderColor} border-2 transition-colors`}
    >
      <div className="text-center">
        ...
        <div
          className={`inline-flex items-center px-4 py-2 rounded-full ${statusInfo.bgColor} ${statusInfo.borderColor} border transition-colors`}
        >
          <span
            className={`text-lg font-semibold ${statusInfo.color} transition-colors`}
          >
        ...
        <div className="mt-6 text-left max-w-3xl mx-auto">
          <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-3 transition-colors">
            📝 해석 가이드 (워렌 버핏 기준)
          </h4>
