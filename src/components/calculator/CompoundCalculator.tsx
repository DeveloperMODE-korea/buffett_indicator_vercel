...
import { calculateCompoundInterest, formatKoreanCurrency } from '@/lib/calculator-utils'
        ...
        <div className="card p-6">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">보수적 (연 5%)</span>
              <span className="font-medium text-gray-900 dark:text-white">
                {formatKoreanCurrency(calculateCompoundInterest({...inputs, annualRate: 5}).finalAmount)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">현재 설정</span>
              <span className="font-medium text-primary-600 dark:text-primary-400">
                {formatKoreanCurrency(result.finalAmount)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">공격적 (연 12%)</span>
              <span className="font-medium text-gray-900 dark:text-white">
                {formatKoreanCurrency(calculateCompoundInterest({...inputs, annualRate: 12}).finalAmount)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="card p-6">
          ...
          <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 transition-colors">
            <p>
              • <strong>{inputs.years}년 동안</strong> 총 <strong>{formatKoreanCurrency(result.totalInvestment)}</strong>을 투자
            </p>
            <p>
              • 연평균 <strong>{inputs.annualRate}%</strong> 수익률로 <strong>{formatKoreanCurrency(result.totalReturn)}</strong> 수익 예상
            </p>
            <p>
              • 복리 효과로 원금 대비 <strong>{result.returnRate.toFixed(1)}%</strong> 성장
            </p>
            <p>
              • 매월 <strong>{formatKoreanCurrency(inputs.monthlyContribution)}</strong> 적립 시 목표 달성
            </p>
          </div>
        </div>
      </div>
